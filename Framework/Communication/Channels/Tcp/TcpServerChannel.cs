...
namespace Trx.Communication.Channels.Tcp
{
    public class TcpServerChannel : BaseServerChannel
    {
        ...
        protected virtual IServerChildChannel CreateChild(Socket acceptedSocket)
        {
            ...
            if (TupleSpace == null)
                child = new TcpServerChildChannel(PipelineFactory.CreatePipeline(), this, acceptedSocket, childName, true)
                {
                    SendMaxRequestSize = _sendMaxRequestSize
                };
            else if (MessagesIdentifier == null)
                child = new TcpServerChildChannel(PipelineFactory.CreatePipeline(), this, acceptedSocket, childName, TupleSpace, true)
                {
                    SendMaxRequestSize = _sendMaxRequestSize
                };
            else
                child = new TcpServerChildChannel(PipelineFactory.CreatePipeline(), this, acceptedSocket, childName,TupleSpace, MessagesIdentifier, true)
                {
                    SendMaxRequestSize = _sendMaxRequestSize
                };
