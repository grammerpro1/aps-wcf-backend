        ...
        protected void StartAsyncSend(AsyncSendInfo sendInfo)
        {
            try
            {
                ...
                if (Logger.IsDebugEnabled())
                    ...
                    Console.WriteLine("###########Trama1: " + StringUtilities.DumpBufferData(string.Format("{0} raw data to send ({1} {2}): ", GetChannelTitle(),buffer.DataLength, buffer.DataLength == 1 ? "byte" : "bytes"),buffer, buffer.LowerDataBound, buffer.DataLength));
                    Console.WriteLine("###########Trama2: " + buffer);
                //Console.WriteLine("###########Trama3: " + buffer.ReadAsString(false));
                //Console.WriteLine("###########Trama4: " + buffer.Read(true));
